...
LocationData = pd.read_csv(
    temp_folder + "/locations.csv",
    index_col=False,
    usecols=["location", "continent", "population"],
)
...
class LocationClass:
    ...
    def AddData(
        self, CasesData, DeathsData, FirstCase, FirstDeath, TotalCases, TotalDeaths
    ):
...
def evalData(Count_arr):
    ...
    for index, count in enumerate(Count_arr):
        ...
        if index < 7:
            ...
            result["SevenDayAvg"].append(
                round(sum(Count_arr[index - 6 : index + 1]) / 7, 3)
            )
...
object_list = list(
    LocationClass(row.location, row.continent, row.population)
